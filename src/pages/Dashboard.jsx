...
import {
  ...
  Clock, Calendar, ChevronRight, Plus, ArrowUpRight, MoreHorizontal,
...
function KPICard({ title, value, change, changeType, icon: Icon, color, subtitle, onClick, trend }) {
  ...
  return (
    <motion.div variants={itemVariants} whileHover={{ y: -4 }} className="h-full">
      <Card className="relative overflow-hidden cursor-pointer group border-0 shadow-lg bg-white h-full" onClick={onClick}>
...
function ActivityItem({ activity, index }) {
  ...
  return (
    <motion.div
      ...
      className="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50 cursor-pointer group transition-colors"
    >
      <div className={`p-2 rounded-lg ${config.color} flex-shrink-0`}>
        <Icon className="h-4 w-4" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-1">{activity.title}</p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {quickActions.map((action, i) => (
                  <motion.button
                    ...
                    className="flex flex-col items-start gap-3 p-4 rounded-xl bg-white shadow-md hover:shadow-xl transition-all duration-300 border border-slate-100 group text-left w-full"
                  >
