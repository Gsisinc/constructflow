...
export async function loadPolicy({ organizationId }) {
  ...
  try {
    const rows = await constructflowClient.get(`/role-permissions?organization_id=${organizationId}`);
...
export async function savePolicy({ organizationId, policy }) {
  ...
  try {
    const existing = await constructflowClient.get(`/role-permissions?organization_id=${organizationId}`);

    await Promise.all(existing.map((row) => constructflowClient.delete(`/role-permissions/${row.id}`)));
    ...
    await Promise.all(records.map((record) => constructflowClient.post('/role-permissions', record)));
  } catch (error) {
